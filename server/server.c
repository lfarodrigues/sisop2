  ...
  do
  {
      ...
      switch (clientRequest.command)
      {
        case UPLOAD: 
			...
			send_to_all_devices(clientRequest.file, client_socket, userid);
			break;
	   case DOWNLOADALL: 
...
void send_to_all_devices(char* filename, int client_socket, char *userid){
	struct client_list *client_node;
	if(findNode(userid, client_list, &client_node)){	
		for(int i = 0; i < MAX_DEVICES; i++){
			if (client_node->client.devices[i] != client_socket){
				send_file(filename, client_node->client.devices[i], userid);
			}
		}
	}
