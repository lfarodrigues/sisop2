  ...
  while (1)
  {
    ...
    if (thread = 0)
    {
      if (pthread_create(&syncThread, NULL, sync_thread_sv, &newsockfd))
      {
        ...
        printf("Commands thread created\n");
      }    
    }
    else if (thread = 1)
    {
...
/*
void assign_recv_socket(int client_socket)
{
  //inclui socket de sync na struct
  if (findNode(userid, client_list, &client))
  {
    if(client->devices[0].socket_sync == FREEDEV){
      client->devices[0].socket_sync = client_socket;
    }
    else if (client->devices[1].socket_sync == FREEDEV){
      client->devices[1].socket_sync = client_socket;
    } else {
      printf("ERROR creating sync socket\n");
    }
  } else {
    printf("ERROR client not found before creating sync socket\n");
  }
}*/
...
int initializeClient(int client_socket, char *userid, struct client *client, int thread)
{
  ...
  struct stat sb;
  int i;
  int *refSocket0, *refSocket1;

  if (thread) {
    refSocket0 = client->devices[0]->socket_cmd;
    refSocket1 = client->devices[1]->socket_cmd;
  } else {
    refSocket0 = client->devices[0]->socket_sync;
    refSocket1 = client->devices[1]->socket_sync;
  }
  // não encontrou na lista ---- NEW CLIENT
  if (!findNode(userid, client_list, &client_node))
  {
    *refSocket0 = client_socket;
    *refSocket1 = -1;
  ...
  {
    if (*refSocket0 == FREEDEV)
    {
      *refSocket0 = client_socket;
    }
    else if (*refSocket1 == FREEDEV)
    {
      *refSocket1 = client_socket;
    }
...
void *sync_thread_sv(void *socket)
{
  ...
  //inclui socket de sync na struct
  initializeClient(*client_socket, userid, &client, 0);
...
void listen_sync(int client_socket, char *userid)
{
  ...
  do
  {
    ...
    {
      case UPLOAD:
        receive_file(clientRequest.file, client_socket, userid);
        break;
      case DOWNLOADALL:
        send_all_files(client_socket, userid);
        break;
      case DELETE:
        delete_file(clientRequest.file, client_socket, userid);
      case EXIT:;
        break;
      default:
        break;
...
void *client_thread(void *socket)
{
  ...
  if (initializeClient(*client_socket, userid, &client, 1) > 0)
  {
...
void listen_client(int client_socket, char *userid)
{
  ...
  do
  {
    ...
    {
    case LIST:
      send_file_info(client_socket, userid);
      break;
    case DOWNLOAD:
      send_file(clientRequest.file, client_socket, userid);
      break;
    case UPLOAD:
      receive_file(clientRequest.file, client_socket, userid);
      break;
    case EXIT:
      close_client_connection(client_socket, userid);
      break;
...
void close_client_connection(int socket, char *userid)
{
  ...
  if (findNode(userid, client_list, &client_node))
  {
    if (client_node->client.devices[0].socket_cmd == FREEDEV)
    {
      client_node->client.devices[1].socket_cmd = FREEDEV;
      client_node->client.logged_in = 0;
    }
    else if (client_node->client.devices[1].socket_cmd == FREEDEV)
    {
      client_node->client.devices[0].socket_cmd = FREEDEV;
      client_node->client.logged_in = 0;
    }
    else if (client_node->client.devices[0].socket_cmd == socket)
      client_node->client.devices[0].socket_cmd = FREEDEV;
    else
      client_node->client.devices[1].socket_cmd = FREEDEV;
  }
...
void receive_file(char *file, int socket, char *userid)
{
  ...
  if (ptrfile = fopen(path, "wb"))
  {
    ...
    // sends the file to sync socket, which is readed in receiver thread in client
    propagate_file(clientRequest.file, socket, userid);
  }
}
...
void send_file(char *file, int socket, char *userid)
{
  ...
  if (ptrfile = fopen(path, "rb"))
  {
    ...
    byteCount = write(socket, file, sizeof(file));
...
void initializeClientList()
{
  ...
  if (d)
  {
    while ((dir = readdir(d)) != NULL)
    {
      ...
      {
        client.devices[0].socket_cmd = FREEDEV;
        client.devices[1].socket_cmd = FREEDEV;
        client.devices[0].socket_sync = FREEDEV;
        client.devices[1].socket_sync = FREEDEV;
        client.logged_in = 0;
...
{
  printf("Entrou aqui\n");
  struct client client = client_list->client;
  while (!strcmp(client.userid, userid) != 0)
  {
    if (client_list->next != NULL)
    {
      client = client_list->next->client;
    }
  }
  for (int i = 0; i <= 2; i++)
  {
    if (client.devices[i].socket_sync == socket || client.devices[i].socket_cmd == socket)
    {
      send_file(file, client.devices->socket_sync, userid);
    }
  }
  printf("Saiu daqui com client %s e socket %d\n", userid, socket);
}
